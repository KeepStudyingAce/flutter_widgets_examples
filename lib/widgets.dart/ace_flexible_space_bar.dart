//系统flexible_space_bar组件，删除不必要的缩放动画
...
/// [AceFlexibleSpaceBar.createSettings], to convey sizing information down to the
/// [AceFlexibleSpaceBar].
///
/// {@tool dartpad --template=freeform}
/// This sample application demonstrates the different features of the
/// [AceFlexibleSpaceBar] when used in a [SliverAppBar]. This app bar is configured
/// to stretch into the overscroll space, and uses the
/// [AceFlexibleSpaceBar.stretchModes] to apply `fadeTitle`, `blurBackground` and
...
class AceFlexibleSpaceBar extends StatefulWidget {
  /// Creates a flexible space bar.
  ///
  /// Most commonly used in the [AppBar.flexibleSpace] field.
  const AceFlexibleSpaceBar({
    Key key,
    this.title,
    this.background,
    this.centerTitle,
  ...
  /// [AceFlexibleSpaceBar] when fully collapsed. `maxExtent` sets the maximum
  /// height of the resulting [AceFlexibleSpaceBar] when fully expanded.
  /// `currentExtent` sets the scale of the [AceFlexibleSpaceBar.background] and
  /// [AceFlexibleSpaceBar.title] widgets of [AceFlexibleSpaceBar] upon
  /// initialization.
  ///
  /// See also:
  ///
  ///  * [AceFlexibleSpaceBarSettings] which creates a settings object that can be
  ///    used to specify these settings to a [AceFlexibleSpaceBar].
  ...
  @override
  _AceFlexibleSpaceBarState createState() => _AceFlexibleSpaceBarState();
}

class _AceFlexibleSpaceBarState extends State<AceFlexibleSpaceBar> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      ...
      assert(
        settings != null,
        'A AceFlexibleSpaceBar must be wrapped in the widget returned by AceFlexibleSpaceBar.createSettings().',
      );
      ...
      if (widget.title != null) {
        ...
        if (opacity > 0.0) {
          ...
          //删除系统缩放动画
          final double scaleValue = Tween<double>(begin: 1.0, end: 1.3)
              .transform(t); //缩放效果修改，系统是Tween<double>(begin: 1.5, end: 1.0)
          final Matrix4 scaleTransform = Matrix4.identity()
            ..scale(1.0, 1.0, 1.0); //不缩放，系统是.scale(scaleValue, scaleValue, 1.0)
...
/// Provides sizing and opacity information to a [AceFlexibleSpaceBar].
///
/// See also:
///
///  * [AceFlexibleSpaceBar] which creates a flexible space bar.
class AceFlexibleSpaceBarSettings extends InheritedWidget {
  /// Creates a Flexible Space Bar Settings widget.
  ///
  /// Used by [Scaffold] and [SliverAppBar]. [child] must have a
  /// [AceFlexibleSpaceBar] widget in its tree for the settings to take affect.
  ///
  /// The required [toolbarOpacity], [minExtent], [maxExtent], [currentExtent],
  /// and [child] parameters must not be null.
  const AceFlexibleSpaceBarSettings({
    Key key,
  ...
  /// Minimum height of the resulting [AceFlexibleSpaceBar] when fully collapsed.
  final double minExtent;

  /// Maximum height of the resulting [AceFlexibleSpaceBar] when fully expanded.
  final double maxExtent;

  /// If the [AceFlexibleSpaceBar.title] or the [AceFlexibleSpaceBar.background] is
  /// not null, then this value is used to calculate the relative scale of
  /// these elements upon initialization.
  final double currentExtent;

  @override
  bool updateShouldNotify(AceFlexibleSpaceBarSettings oldWidget) {
